...
describe('roundToQuarterHour', () => {
  ...
  test('1-7 minutes past a quarter rounds down', () => {
    [1, 4, 7].forEach(min => {
  ...
  test('8-14 minutes past a quarter rounds up', () => {
    [8, 11, 14].forEach(min => {
      const d = roundToQuarterHour(new Date(2026, 8, 11, 9, min, 0));
      assert.equal(d.getHours(), 9, `minute ${min}`);
  ...
  test('exact 7:30 tie rounds up (DOL rule: 7:59 and below is the down side)', () => {
    const d = roundToQuarterHour(new Date(2026, 8, 11, 9, 7, 30));
    assert.equal(d.getHours(), 9);
    assert.equal(d.getMinutes(), 15);
  });

  test('rounds up across an hour boundary (9:53 -> 10:00)', () => {
...
describe('recHoursRounded', () => {
  test('rounds both punches before diffing (9:11 -> 9:15, 12:53 -> 13:00 => 3h45m)', () => {
    const r = {
      clock_in: new Date(2026, 8, 11, 9, 11, 0).toISOString(),
      clock_out: new Date(2026, 8, 11, 12, 53, 0).toISOString()
    };
    assert.equal(recHoursRounded(r), 3.75);
  });
  ...
  test('a short session that rounds to the same instant on both ends -> 0, not negative', () => {
    const r = {
      clock_in: new Date(2026, 8, 11, 9, 7, 0).toISOString(),  // rounds down to 9:00
      clock_out: new Date(2026, 8, 11, 9, 6, 0).toISOString()  // rounds down to 9:00
    };
